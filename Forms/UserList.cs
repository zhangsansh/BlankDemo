...
namespace BlankDemo.Forms
{
    public partial class UserList : Form
    {
        ...
        {
            // 1. 构建查询对象
            var query = SqlSugerHepler.Db.Queryable<UserInfo>();
            ...
            {
                query = query.Where(it => it.Account.Contains(keyword));
            }
        ...
        private void button9_Click(object sender, EventArgs e)
        {
                ...
                "确认批量删除", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    ...
                    int count = SqlSugerHepler.Db.Updateable<UserInfo>()
                        .SetColumns(it => new UserInfo
                        {
                            // 在这里写你要更新的字段（可多个）
                            Status = 2,              // 例：状态改为2
                                  // 例：禁用账号
                        ...
                        .Where(it => idList.Contains(it.UserId)) // 只更新选中的ID
                        .ExecuteCommand();
        ...
        {
            int id = int.Parse(dataGridView1.SelectedRows[0].Cells["UserId"].Value.ToString());
            UserInfo info = SqlSugerHepler.Db.Queryable<UserInfo>().Where(it => it.UserId == id).First();
