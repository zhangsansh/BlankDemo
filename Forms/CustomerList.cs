...
namespace BlankDemo.Forms
{
    public partial class CustomerList : Form
    {
        ...
        {
            // 1. 构建查询对象
            var query = SqlSugerHepler.Db.Queryable<CustomerInfo>();
            ...
            {
                query = query.Where(it => it.CustomerName.Contains(keyword));
            }
            ...
            dataGridView1.DataSource = query
                ...
                .OrderByDescending(it => it.CustomerId)
                ...
                "确认批量删除", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    ...
                    foreach (DataGridViewRow row in dataGridView1.SelectedRows)
                    {
                        ...
                        int AddressId = Convert.ToInt32(row.Cells["CustomerId"].Value);
                        idList.Add(AddressId);
                    }

                    // ====================== 【核心：批量更新字段】 ======================
                    // 这里可以改任意字段！示例：更新状态、姓名、性别、时间、开关等
                    int count = SqlSugerHepler.Db.Updateable<CustomerInfo>()
                        .SetColumns(it => new CustomerInfo
                        {
                            // 在这里写你要更新的字段（可多个）
                            Status = 0,              // 例：状态改为2
                                                     // 例：禁用账号
                        ...
                        .Where(it => idList.Contains(it.CustomerId)) // 只更新选中的ID
                        .ExecuteCommand();
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            CustomerInfo info = SqlSugerHepler.Db.Queryable<CustomerInfo>().Where(it => it.CustomerId == id).First();
            //CustomerInfo info = new CustomerInfo();
            CustomerAdd customerAdd = new CustomerAdd(true, info);
            if (customerAdd.ShowDialog() == DialogResult.OK)
            {
                LoadData();
            }
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            //CustomerInfo info = SqlSugerHepler.Db.Queryable<CustomerInfo>().Where(it => it.CustomerId == id).First();
            CustomerInfo info = new CustomerInfo();
            CustomerAdd customerAdd = new CustomerAdd(false, info);
            if (customerAdd.ShowDialog() == DialogResult.OK)
            {
                LoadData();
            }
